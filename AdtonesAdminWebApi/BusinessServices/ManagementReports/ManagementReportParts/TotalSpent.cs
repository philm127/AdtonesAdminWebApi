...
namespace AdtonesAdminWebApi.BusinessServices.ManagementReports.ManagementReportParts
{
    ...
    public interface ITotalSpent
    {
        Task<TotalCostCredit> GetTotalTotalSpent(ManagementReportsSearch search, int ops, string conn);
    }

    public class TotalSpent : BaseDAL, ITotalSpent
    {
        private readonly ILoggingService _logServ;
        private readonly ICalculateConvertedSpendCredit _convertedSpend;

        public TotalSpent(ILoggingService logServ, IConfiguration configuration, IExecutionCommand executers, IConnectionStringService connService,
        ...
        public async Task<TotalCostCredit> GetTotalTotalSpent(ManagementReportsSearch search, int ops, string conn)
        {
        ...
        private static string GetTotalSpent => @"SELECT ISNULL(SUM(cp.TotalCost),0) AS TotalCost,0 AS TotalCredit,cur.CurrencyCode 
