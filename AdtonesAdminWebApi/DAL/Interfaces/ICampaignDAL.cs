...
namespace AdtonesAdminWebApi.DAL.Interfaces
{
    public interface ICampaignDAL
    {
        ...
        Task<IEnumerable<CampaignAdminResult>> GetAdminOpAdminCampaignResultSet();
        Task<CampaignAdminResult> GetCampaignResultSetById(int id);
        // Task<CampaignProfile> GetCampaignProfileDetail(int id = 0);
        Task<CampaignProfileDto> GetCampaignProfileDetail(int campaignId);
        Task<int> ChangeCampaignProfileStatus(int campaignProfileId, int status);
        Task<int> ChangeCampaignProfileStatusOperator(int campaignProfileId, int status, int operatorId);
        ...
        Task<IEnumerable<CampaignAdminResult>> GetCampaignResultSetBySalesExec(IEnumerable<int> numbers);
        Task<IEnumerable<CampaignAdminResult>> GetCampaignResultSetByAdvertiser(int id);

        Task<int> UpdateCampaignCredit(CampaignCreditCommand model, List<string> conStrLst);
